...
if [ -z "$PYTHON" ] && command -v uv >/dev/null 2>&1; then
    ...
    if [ -n "$_UV_TOOL_DIR" ] && [ -x "$_UV_TOOL_DIR/graphifyy/bin/python" ]; then
        _UV_PY="$_UV_TOOL_DIR/graphifyy/bin/python"
        "$_UV_PY" -c "import graphify" 2>/dev/null && PYTHON="$_UV_PY"
    fi
fi
...
```
Graph complete. Outputs in CURRENT_WORKING_DIRECTORY/graphify-out/
...
Replace CURRENT_WORKING_DIRECTORY with the absolute path of the directory where the skill was invoked. The skill pipeline writes `graphify-out/` relative to that directory, even when INPUT_PATH points elsewhere.
