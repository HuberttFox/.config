...
function getStateDir(client = 'opencode') {
  let root;
  if (process.env.CAVEMAN_STATE_HOME) {
    root = process.env.CAVEMAN_STATE_HOME;
  } else if (process.env.XDG_STATE_HOME) {
    root = path.join(process.env.XDG_STATE_HOME, 'caveman');
    ...
    root = path.join(os.homedir(), '.local', 'state', 'caveman');
  }
  return client ? path.join(root, client) : root;
}

function getFlagPath(client = 'opencode') {
  return path.join(getStateDir(client), 'active');
}

function getHistoryPath(client = 'opencode') {
  return path.join(getStateDir(client), 'usage.jsonl');
}
...
// Symlink-safe history append. Same parent-dir + symlink-target rules as
// safeWriteFlag, but opens with O_APPEND so concurrent OpenCode sessions do not
// clobber each other. Rotates at 8 MiB and retains one prior segment.
function safeAppendHistory(filePath, line) {
  const debug = process.env.CAVEMAN_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[caveman] safeAppendHistory: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
    ...
    try {
      ...
      if (st.size >= MAX_HISTORY_BYTES) {
        const rotatedPath = `${realPath}.1`;
        try {
          const rotatedStat = fs.lstatSync(rotatedPath);
          if (rotatedStat.isSymbolicLink()) return;
          fs.unlinkSync(rotatedPath);
        } catch (e) {
          if (e.code !== 'ENOENT') return;
        }
        fs.renameSync(realPath, rotatedPath);
      }
    } catch (e) {
      if (e.code !== 'ENOENT') return;
    }
...
// Symlink-safe history read. Returns lines from the current and retained prior
// segment, or an empty array on any anomaly. Each segment is capped by rotation.
function readHistory(filePath) {
  ...
  for (const candidate of paths) {
    try {
      ...
      if (st.isSymbolicLink() || !st.isFile() || st.size > MAX_HISTORY_BYTES) continue;
      const O_NOFOLLOW = typeof fs.constants.O_NOFOLLOW === 'number' ? fs.constants.O_NOFOLLOW : 0;
      const flags = fs.constants.O_RDONLY | O_NOFOLLOW;
      let fd;
      let raw;
