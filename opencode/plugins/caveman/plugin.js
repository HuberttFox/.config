...
const flagPath = getFlagPath('opencode');
const historyPath = getHistoryPath('opencode');
const legacyFlagPath = path.join(opencodeConfigDir(), '.caveman-active');
...
function parseModeChange(promptRaw) {
  ...
  if (tpl) {
    ...
    if (arg === 'wenyan-full') return 'wenyan-full';
...
function migrateLegacyFlag() {
  if (readFlag(flagPath)) return;
  const legacyMode = readFlag(legacyFlagPath);
  if (legacyMode && safeWriteFlag(flagPath, legacyMode)) removeFlag(legacyFlagPath);
}
...
function applyModeChange(mode) {
  ...
  if (mode === 'off') {
    removeFlag(flagPath);
    removeFlag(legacyFlagPath);
    return;
  }
  if (safeWriteFlag(flagPath, mode)) removeFlag(legacyFlagPath);
}
...
function parseUsageRecord(line) {
  try {
    ...
    if (record && record.schema === 1 && record.type === 'turn_usage' &&
        record.messageId && record.usage) return record;
  } catch (e) {}
  return null;
}

function aggregateUsage(sessionID) {
  const records = [];
  const seen = new Set();
  ...
  function summarize(items) {
    ...
    for (const item of items) {
      if (item.reinforcementApplied) totals.active++;
      totals.input += Number(item.usage.input) || 0;
      totals.output += Number(item.usage.output) || 0;
      totals.reasoning += Number(item.usage.reasoning) || 0;
      totals.cacheRead += Number(item.usage.cacheRead) || 0;
      totals.cacheWrite += Number(item.usage.cacheWrite) || 0;
  ...
  return {
    session: summarize(records.filter((record) => record.sessionId === sessionID)),
    lifetime: summarize(records),
  };
}

function formatUsageStats(sessionID) {
  const stats = aggregateUsage(sessionID);
  const number = new Intl.NumberFormat('en-US');
  const row = (label, value) => [
    label.padEnd(12),
    String(value.turns).padStart(6),
    String(value.active).padStart(7),
    number.format(value.input).padStart(10),
    number.format(value.output).padStart(10),
    number.format(value.reasoning).padStart(11),
    number.format(value.cacheRead).padStart(12),
    ('$' + value.cost.toFixed(4)).padStart(10),
  ].join(' ');

  return [
    'Observed OpenCode usage',
    '',
    'Scope         Turns  Active      Input     Output   Reasoning   Cache read       Cost',
    row('Session', stats.session),
    row('Lifetime', stats.lifetime),
    '',
    'OpenCode hooks cannot measure counterfactual token savings.',
  ].join('\n');
}

const MAX_TRACKED_MESSAGES = 2048;

function rememberBounded(collection, key, value) {
  collection.delete(key);
  collection.set(key, value);
  if (collection.size > MAX_TRACKED_MESSAGES) {
...
export const CavemanPlugin = async (_ctx) => {
  ...
  event: async ({ event } = {}) => {
    ...
    if (!info || info.role !== 'assistant' || !info.time || !info.time.completed ||
        recordedMessages.has(info.id)) return;

    const attribution = pendingModes.get(info.parentID);
    const record = {
      schema: 1,
      type: 'turn_usage',
      ...
      mode: attribution ? attribution.mode : null,
      reinforcementApplied: attribution ? attribution.reinforcementApplied : null,
      model: {
        providerId: info.providerID,
        modelId: info.modelID,
      },
      usage: {
        input: info.tokens && info.tokens.input || 0,
        output: info.tokens && info.tokens.output || 0,
        reasoning: info.tokens && info.tokens.reasoning || 0,
        cacheRead: info.tokens && info.tokens.cache && info.tokens.cache.read || 0,
        cacheWrite: info.tokens && info.tokens.cache && info.tokens.cache.write || 0,
      },
      cost: info.cost || 0,
      finish: info.finish || null,
    };
    safeAppendHistory(historyPath, JSON.stringify(record));
    rememberBounded(recordedMessages, info.id, true);
  },
  ...
  'chat.message': async (input, output) => {
    ...
    const messageID = output.message && output.message.id || input && input.messageID;
    if (messageID) {
      const mode = readFlag(flagPath) || 'off';
      rememberBounded(pendingModes, messageID, {
        mode,
        reinforcementApplied: mode !== 'off' && !INDEPENDENT_MODES.has(mode),
      });
    }
  },
  ...
  tool: {
    caveman_stats: tool({
      description: 'Show factual OpenCode token usage recorded by Caveman.',
      args: {},
      async execute(_args, context) {
        return formatUsageStats(context.sessionID);
      },
    }),
  },
