...
EOF
  ...
  curl)
    ...
    [[ -n "$out" ]] && { mkdir -p "$(dirname "$out")"; printf '# stub zimfw\n' > "$out"; } ;;
  zsh)
...
for name in uname brew git curl zsh dscl sudo chsh tmux fzf starship lazygit vim yazi; do
...
run_installer --only zsh,fzf >/dev/null
...
printf 'original\n' > "$HOME/.zshrc"
run_installer --only zsh >/dev/null
